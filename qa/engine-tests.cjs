...
function tests(){
 ...
 assert.equal(d.NINJAS.length,55);assert.equal(d.RELICS.length,36);assert.equal(d.ENCOUNTERS.length,42);
 ...
 g.start();t.setState({encounterIndex:41,playsLeft:1,relics:[],score:0,training:0});g.toggleCard(t.state().hand[0].uid);g.play();e.flush();assert.equal(t.state().phase,'gameover');assert(!e.storage['shinobi-cascade-v3']);
 ...
 // Returning home saves settled state; reopening cannot repeat rewards or reroll shops.
 g.start('leaf',123);const snapshot=JSON.stringify(t.state().hand);g.home();assert.equal(t.state().phase,'menu');g.resume();assert.equal(JSON.stringify(t.state().hand),snapshot);
 forceWin('treasure_frog');const shop=JSON.stringify(t.state().shopItems),cash=t.state().money;g.home();g.resume();assert.equal(t.state().phase,'shop');assert.equal(JSON.stringify(t.state().shopItems),shop);assert.equal(t.state().money,cash);
 g.start('leaf',123);g.toggleCard(t.state().hand[0].uid);g.play();g.home();e.flush();assert.equal(t.state().phase,'menu');g.resume();assert.equal(t.state().playsLeft,3);assert(t.state().score>0);
 for(const [rule,ids] of [['precision',['naruto']],['reversal',['sasuke']],['silence',['naruto']],['adaptation',['naruto']]]){set({ruleOverride:null,previousCount:1});const base=t.evaluate(ids).damage;set({ruleOverride:rule,previousCount:1});assert(t.evaluate(ids).damage<base,rule);}
