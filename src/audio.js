...
const SFX = (() => {
  let ctx = null;
  const PREF_KEY = 'shinobi-cascade-sound';
  ...
  return {
    ...
    toggle(){enabled=!enabled;try{localStorage.setItem(PREF_KEY,enabled?'on':'off')}catch(e){} if(enabled) tone(520,.05,'sine',.04); return enabled;},
